...
from src.common.app_logger import AppLogger, get_disabled_logger
...
class TestAppLogger(unittest.TestCase):
    ...
    def setUpClass(cls):
        cls.valid_config = {
            ...
            "app_insights_key": "InstrumentationKey=" + test_instrumentation_key,
        }
        cls.invalid_config = {
            "log_level": "DEBUG",
            "logging_enabled": "false",
            "app_insights_key": "InstrumentationKey="
            + test_invalid_instrumentation_key,
        }
    ...
    def test_log_helper_raising_exception(self):
        mock = Mock()
        mock.side_effect = Exception("Testing log helper exception")
        with self.assertRaises(Exception):
            log_helper.get_logger(mock)
        print("LogHelper - Test - raising of unhandled exception")

    @mock.patch.dict(
        os.environ, {"APPINSIGHTS_CONNECTION_STRING": "dummy value"}, clear=True
    )
    def test_log_helper_verify_custom_dimensions(self):
        log_helper.app_insights_key = "dummy value"
        log_helper.device_id = "dummy value"
        log_helper.module_id = "dummy value"
        with self.assertRaises(Exception):
            log_helper.get_logger()
        print(
            "LogHelper - Test - verify exception when initializing app insights logging"
        )
